  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <nav className="bg-white border-b border-gray-200">
        ...
        {/* Mobile nav */}
        <div className="sm:hidden border-t border-gray-100 px-3 py-2 flex flex-wrap gap-1">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="px-3 py-2 text-sm text-gray-700 rounded-md hover:bg-gray-100 active:bg-gray-200"
            >
              {link.label}
            </Link>
          ))}
