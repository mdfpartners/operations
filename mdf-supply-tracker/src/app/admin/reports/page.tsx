  ...
  return (
    <div className="space-y-6">
      ...
      <form method="GET" className="flex flex-wrap items-end gap-3 bg-white rounded-lg shadow-sm p-4">
        ...
        {(fromDate || toDate) && <a href="/admin/reports" className="text-sm text-gray-500 hover:text-gray-700 py-1.5">Clear</a>}
        {(fromDate || toDate) && <span className="text-xs text-blue-700 py-1.5">Showing {fromDate || 'all time'} → {toDate || 'today'}</span>}
      </form>

      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="bg-white rounded-lg shadow-sm p-4"><p className="text-xs text-gray-500">Open Orders</p><p className="text-3xl font-bold text-gray-900">{open.length}</p></div>
        <div className="bg-white rounded-lg shadow-sm p-4"><p className="text-xs text-gray-500">8+ Day Open</p><p className="text-3xl font-bold text-red-700">{eightPlusOpen.length}</p></div>
        <div className="bg-white rounded-lg shadow-sm p-4"><p className="text-xs text-gray-500">Emergency / OOS Open</p><p className="text-3xl font-bold text-orange-700">{emergencyOpen.length}</p></div>
        <div className="bg-white rounded-lg shadow-sm p-4"><p className="text-xs text-gray-500">Avg Days to Complete</p><p className="text-3xl font-bold text-gray-900">{avgDays ?? '—'}</p></div>
      </div>
      ...
      <div className="grid sm:grid-cols-2 gap-6">
        <div className="bg-white rounded-lg shadow-sm p-5">
          ...
          <table className="w-full text-sm"><tbody className="divide-y divide-gray-100">
            {Object.entries(byStatus).sort((a, b) => b[1] - a[1]).map(([s, c]) => (<tr key={s}><td className="py-1.5 text-gray-700">{STATUS_LABELS[s as OrderStatus]}</td><td className="py-1.5 font-medium text-right">{c}</td></tr>))}
        ...
        <div className="bg-white rounded-lg shadow-sm p-5">
          ...
          <table className="w-full text-sm"><tbody className="divide-y divide-gray-100">
            {['0','1','2','3','4-7','8+'].map((b) => (<tr key={b}><td className="py-1.5 text-gray-700">{b} day(s)</td><td className="py-1.5 font-medium text-right">{byAging[b] || 0}</td></tr>))}
          </tbody></table>
        </div>
        <div className="bg-white rounded-lg shadow-sm p-5">
          <h2 className="font-semibold text-gray-900 mb-3">Spend by Account</h2>
          <table className="w-full text-sm"><tbody className="divide-y divide-gray-100">
            {Object.values(spendByAccount).sort((a, b) => b.total - a.total).map((a) => (<tr key={a.name}><td className="py-1.5 text-gray-700">{a.name}</td><td className="py-1.5 font-medium text-right">${a.total.toFixed(2)}</td></tr>))}
        ...
        <div className="bg-white rounded-lg shadow-sm p-5">
          ...
          <table className="w-full text-sm"><tbody className="divide-y divide-gray-100">
            {Object.values(spendByVendor).filter(v => v.total > 0).sort((a, b) => b.total - a.total).map((v) => (<tr key={v.name}><td className="py-1.5 text-gray-700">{v.name}</td><td className="py-1.5 font-medium text-right">${v.total.toFixed(2)}</td></tr>))}
        ...
        <div className="bg-white rounded-lg shadow-sm p-5">
          ...
          <table className="w-full text-sm"><tbody className="divide-y divide-gray-100">
            {Object.entries(spendByCategory).filter(([, v]) => v > 0).sort((a, b) => b[1] - a[1]).map(([cat, total]) => (<tr key={cat}><td className="py-1.5 text-gray-700">{cat}</td><td className="py-1.5 font-medium text-right">${total.toFixed(2)}</td></tr>))}
        ...
        <div className="bg-white rounded-lg shadow-sm p-5">
          ...
          <table className="w-full text-sm"><tbody className="divide-y divide-gray-100">
            {Object.values(emergencyByRequester).sort((a, b) => b.count - a.count).map((r) => (<tr key={r.name}><td className="py-1.5 text-gray-700">{r.name}</td><td className="py-1.5 font-medium text-right">{r.count}</td></tr>))}
        ...
        <div className="bg-white rounded-lg shadow-sm p-5">
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {delayed.map((o) => (<tr key={o.id}><td className="py-1.5"><Link href={`/admin/orders/${o.id}`} className="text-blue-600 hover:underline">{o.order_number}</Link></td><td className="py-1.5 text-gray-600">{(o as any).account?.name}</td><td className="py-1.5 text-gray-500">{new Date(o.submitted_at).toLocaleDateString()}</td></tr>))}
            </tbody>
          </table>
        </div>
      )}
